...
**Пересмотр UI (в этой же сессии, до начала реализации)**: отдельной вкладки «Shelf» не будет — пользователь решил не плодить вкладки, а докладывать связанный функционал на уже существующие, по образцу `AliasesTab` (`AliasesSection`+`VariablesSection` на одной вкладке). Управление сущностями/workflow (CRUD `EntityType`, редактор `statuses`) переезжает на вкладку бывших «Тегов» вторым разделом, рядом с `TagsSection`. Присвоение сущности+статуса конкретной закладке и её просмотр — там же, где теги (`BookmarkCard`/`LibraryTab`, см. `SEARCH` в `specification.md`), а не на отдельной вкладке.
...
- «Поиск» → **«Библиотека»** (`SearchTab.tsx`→`LibraryTab.tsx`, `nav.search`→новый `nav.library`, иконка `IconSearch`→новая `IconLibrary`) — чтобы явно показать, что это управление всем содержимым (закладки+теги+сущности/статусы), а не только поиск. Попап (`PopupSearch.tsx`) **не** переименован, там остаётся буквально «Поиск» — компактный экран поиска внутри попапа, отдельный контекст, использует прежний ключ `nav.search`.
- «Теги» → **«Категории»** (`TagsTab.tsx`→`CategoriesTab.tsx`, `nav.tags`→`nav.categories`, `tagsTab.*`→`categoriesTab.*`) — новое имя нужно уже сейчас, раз вкладка становится домом не только для тегов, но и для сущностей/workflow.
- «Алиасы» → **«Сопоставления»** (`AliasesTab.tsx`→`MappingsTab.tsx`, `nav.aliases`→`nav.mappings`, `aliasesTab.*`→`mappingsTab.*`) — и алиас (домен→имя), и переменная извлечения (селектор→поле) — по сути именованные соответствия, общий термин описывает оба раздела вкладки честнее, чем «Алиасы».

Глубокая спецификация `SHELF-1-entity-workflow/description.md` (раздел UI) обновлена под всё это.
...
**Доработка UI (в той же сессии, после ревью)**: круглая кнопка-карандаш (`IconEdit`, внутри теговго ряда, `margin-left: auto`) и выпадающий список с `align="end"` (открывался под кнопкой, у правого края) заменены. Пользователю показаны 4 наброска в Claude Artifact (варианты A–D, отличались стилем кнопки-триггера и стилем панели); выбран **вариант A**: кнопка-триггер — текстовая ссылка «Редактировать» акцентным цветом (`var(--accent)`), в правом верхнем углу карточки (в `topRow`, рядом с фавиконкой/заголовком, а не в теговом ряду); попап тегов теперь открывается у **левого** края — там, где начинается ряд тегов, — а не под кнопкой-триггером (которая физически находится в другом углу карточки).

Реализовано: переключено с `@radix-ui/react-dropdown-menu` на **`@radix-ui/react-popover`** (новая зависимость) — у `DropdownMenu` нет публичного `Anchor`, позиция попапа жёстко привязана к триггеру; у `Popover` есть `Popover.Anchor`, что и даёт нужную развязку триггер/позиция без потери доступности (`Popover.Trigger` остаётся настоящей кнопкой с фокус-менеджментом). Чек-лист тегов теперь — простые кнопки `role="checkbox"` внутри `PopoverContent` (не `DropdownMenuCheckboxItem` — у Popover нет built-in меню-семантики, но список тегов концептуально ближе к форме/фильтру, чем к меню команд). Новые файлы: `components/ui/popover.tsx` (тонкая обёртка по образцу `dropdown-menu.tsx`), `hooks/useBookmarkTagEditor.ts` (вынесенная комбинация `useTags`+`useBookmarkTagLink`, переиспользуется старым и новым UI), `components/bookmark/tags/{BookmarkTagEditTrigger,BookmarkTagList}.tsx`. Область — только `BookmarkCard.tsx` (вкладка «Поиск» в настройках); `CompactBookmarkCard.tsx` (поиск в попапе) намеренно не тронут — отдельная, узкая однострочная раскладка, будет переработана отдельной задачей. Старый `BookmarkTagEditor.tsx` (иконка-карандаш + `DropdownMenu`) оставлен как есть, используется только `CompactBookmarkCard`.
