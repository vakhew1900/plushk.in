...
- `BookmarkFavicon` (карточка «Библиотеки»/попап «Поиск») — изначально новый опциональный проп `bookmarkId` (читал `IconBookmark` через хук `useIconLink`/`IIconLinkService.resolveForBookmark` самостоятельно), с фолбэком на прежнее поведение (favicon/буква). **Заменено при доработке `UI-15`** (см. её «Реализовано» ниже) на чисто презентационный проп `iconUrl` — резолвинг переехал в общий `useBookmarkIcon`, вызываемый один раз в `BookmarkCard`/`CompactBookmarkCard`.
...
- `npm run compile`/`npm run lint`/`npm test` (263/263) и `npm run build` — чисто на каждом шаге.

**Не проверено формальным чек-листом:** пользователь тестировал в браузере итеративно в ходе сессии и нашёл/поправил 7 реальных багов выше — но это не то же самое, что прогон по чек-листу `specs/verification.md` (`UI-15`, 8 пунктов) целиком. Чек-лист остаётся неотмеченным до явного подтверждения по каждому пункту; переносить в `changelog.md` рано.
...
- `npm run compile`/`npm run lint`/`npm test` (287/287) и `npm run build` — чисто.

**Не проверено в этой итерации:** визуально в запущенном расширении (нет браузерного тулинга в этой сессии) — 4 новых пункта в `specs/verification.md` (`RULE-8`) требуют ручной проверки через `npm run dev:firefox` перед переносом в `changelog.md`.
