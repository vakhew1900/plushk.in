...
Открытая, ещё не расспрошенная задача — пользователь обозначил общее намерение сделать интерфейс визуально приятнее, не называя конкретных решений: возможный пересмотр текущей пары шрифтов (`Hanken Grotesk` для интерфейса + `JetBrains Mono` для кода/данных, `assets/globals.css`), возможная замена акцентного/палитрового цвета (`--accent`, 8-цветовая `--palette-*` палитра тегов/сущностей/статусов) и другая некрупная визуальная полировка, пока безымянная. Отличается от `UI-6`/`UI-7` (те ввели согласованную шкалу размеров иконок/типографики — систему токенов, но не сами значения шрифта/цвета) — эта задача про пересмотр самих значений, не про инфраструктуру, которая их держит.

На момент постановки ничего не решено и не расспрошено — нужен отдельный интервью-проход (какие именно элементы не нравятся, есть ли референсы/направление, ограничивается ли задача заменой значений токенов или требует более глубокого редизайна) перед тем, как заводить глубокую спецификацию или приступать к реализации.
