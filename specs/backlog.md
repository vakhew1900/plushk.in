...
### SEARCH-4 — Присвоение тегов закладке: quick-save popup + кнопка «редактировать» на карточке в Search
**Priority:** Medium
**Added:** 2026-08-11

Продолжение `SEARCH-3` (хранение тегов уже есть — `Tag`/`ITagRepository`/вкладка «Теги», см. `changelog.md`). У закладки может быть сразу несколько тегов (в отличие от `EntityType` из `SHELF-1`, где связь 0-или-1) — значит нужна отдельная many-to-many связь `bookmarkTags: { bookmarkId, tagId }` (составной ключ `[bookmarkId+tagId]`), а не переиспользование `BookmarkEntityLink`. Присвоение — в двух местах: (1) в попапе quick-save при сохранении (multi-select вместо `RadioGroup`, который `SHELF-1` использует для сущности — там 0-или-1); (2) кнопка «редактировать» на карточке закладки во вкладке «Поиск», открывающая редактор тегов **и** (после `SHELF-1`) сущности/статуса для уже сохранённой закладки — единая точка правки метаданных постфактум. На момент постановки не решено: UI редактора по кнопке «редактировать» (модалка/инлайн-раскрытие карточки — в проекте пока нет `@radix-ui/react-dialog`, нужно ли заводить); нужен ли multi-select компонент поверх Radix (`@radix-ui/react-toggle-group`? кастомные чекбоксы?) — Radix `RadioGroup` не подходит, он single-select. Каскад при удалении тега — `TagRepository.remove()` должен чистить связанные строки `bookmarkTags` (по аналогии с `EntityTypeRepository.remove()` из `SHELF-1`).
...
Исследовательская задача (без изменения кода) — по запросу пользователя, увидевшего непоследовательные размеры крестика (`IconX`) и заподозрившего, что проблема шире. Аудит подтвердил: все 15 иконок в `src/components/icons/` принимают сырой `size?: number` (px), и на местах использования расставлены произвольные числа (10–22, без общей шкалы) — не только у `IconX`. Аналогично `font-size` объявлен вручную в 70+ местах в 38 CSS-модулях, без единого набора токенов, вперемешку `px`/`rem`. Результат — задокументированные варианты решения (CSS custom properties, семантический `size`-проп по образцу уже существующего `ButtonSize`, обёртки-компоненты для типографики) с оценкой трудозатрат и открытыми вопросами для пользователя. **Важно:** когда до этого дойдёт реализация, она обязана также обновить `CLAUDE.md` (секция «Code rules», правило «SVG icons must be extracted as components») и/или пункт про иконки в скилле `scaffold` (`.claude/skills/scaffold/reference/component.md`, правило 8) — прописать там новую конвенцию использования размера иконок (семантический `size`, а не сырое число), иначе новый код продолжит писаться по старому паттерну и решение не приживётся. Глубокая спецификация — `specs/tasks/UI-6-icon-typography-sizing-tokens/`.
