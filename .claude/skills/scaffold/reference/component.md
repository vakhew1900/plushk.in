...
7. Prefer Radix UI primitives for interactive patterns (`RadioGroup`, `Switch`, `Slot`/`asChild`, etc.) and check `src/components/ui/` (thin Radix wrappers: Button, Badge, Switch, Input, Textarea, plus the shared `Text` typography component) before writing a custom interactive element.
8. Never write inline `<svg>` in the component's JSX. If an icon is needed, create or reuse a named component in `src/components/icons/` (e.g. `IconPlus`, `IconFolder`) and import it. Size it with the semantic `size` prop (`'sm' | 'md' | 'lg'`, default `'md'`) — never pass a raw number like `size={13}`. Also never fake an icon with a bare text glyph (`<button>×</button>`, `<button>+</button>`) — that skips the size system and its stroke weight won't match the rest of the icon set; use the matching icon component instead (`IconX` for close/remove, `IconPlus` for add, etc.). For any heading/label/body text in the new component, use `<Text size="..." />` from `src/components/ui/text.tsx` instead of a raw `font-size` in the `.module.css` — see `size`/`weight`/`tone`/`as` in `CLAUDE.md`'s Code rules.
