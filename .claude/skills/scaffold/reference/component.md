...
8. Never write inline `<svg>` in the component's JSX. If an icon is needed, create or reuse a named component in `src/components/icons/` (e.g. `IconPlus`, `IconFolder`) and import it. Size it with the semantic `size` prop (`'sm' | 'md' | 'lg'`, default `'md'`) — never pass a raw number like `size={13}`. For any heading/label/body text in the new component, use `<Text size="..." />` from `src/components/ui/text.tsx` instead of a raw `font-size` in the `.module.css` — see `size`/`weight`/`tone`/`as` in `CLAUDE.md`'s Code rules.
