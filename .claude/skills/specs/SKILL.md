---
name: specs
description: Maintain the project's specification and task-tracking files in specs/ (specification.md, plan.md, backlog.md, tasks.md, changelog.md, cancelled.md, ideas.md), and optionally a deep per-task spec under specs/tasks/<id>/. Use when adding, starting, finishing, cancelling, removing, or re-prioritizing a task, capturing or promoting an idea, updating the spec/plan, or writing a detailed specification for a specific task before implementation.
---
...
- `specs/cancelled.md` — tasks that won't be done (superseded, obsolete, or explicitly out of scope), kept for history.
...
Read all six `specs/*.md` task-tracking files (`backlog.md`, `tasks.md`, `changelog.md`, `cancelled.md`, `ideas.md`, `specification.md`) before adding, moving, or removing a task — a task's category, number, and status depend on the current content of more than one of them.

## Rules for managing tasks

1. Every task has an ID `<CATEGORY>-<N>`, where `<CATEGORY>` is one of the tags listed in the `specs/specification.md` task-categories table (`RULE`, `AI`, `UI`, ...) and `<N>` is sequential *within that category*. Check the highest existing `<N>` for the category across **all four** of `backlog.md`, `tasks.md`, `changelog.md`, and `cancelled.md` before assigning the next number — a cancelled task's ID is retired, never reused.
2. A new task always starts in `backlog.md` — never add a task directly to `tasks.md`, `changelog.md`, or `cancelled.md`.
3. A task lives in exactly one of `backlog.md` / `tasks.md` / `changelog.md` / `cancelled.md` at a time. Moving it means cutting the entry from one file and pasting it into the other — never leave it in two places, never duplicate it.
...
6. When a task becomes obsolete — superseded by what another task ended up implementing, the scenario it was designed for stops existing, or it's explicitly decided out of scope — move its entry from `backlog.md` or `tasks.md` to `cancelled.md` instead of deleting it, appending **Cancelled:** (`YYYY-MM-DD`) and **Reason:** fields. Same append-only rule as `changelog.md`. Don't cancel a task just because it looks stale or low-priority — cancel it because the described work genuinely doesn't need doing; if unsure, ask the user rather than deciding unilaterally.
7. Every `backlog.md`/`tasks.md` entry has all four fields — ID, Priority (`low` / `medium` / `high` / `critical`), Added (`YYYY-MM-DD`), Description — in the exact format already used there (template in `CLAUDE.md`). `changelog.md` entries have those four plus `Completed`. `cancelled.md` entries have those four plus `Cancelled` and `Reason`.
8. If a task needs a category that isn't in the `specs/specification.md` table yet, add the new tag with a short description there first, then use it.
9. Don't rewrite, reformat, or reorder existing entries beyond what the requested change actually requires.
10. Mirror every add/move/remove/cancel to the Obsidian Kanban board at `projects/plusk.in/mvp kanban.md` (via the `obsidian` MCP server — `vault_patch`/`vault_read` on that path) in the same turn as the `specs/*.md` edit, so the two never drift. `specs/*.md` stays the single source of truth; the board is a read-mostly reflection of it. Five lanes, exact heading text (needed verbatim for `vault_patch` heading targets — the trailing tag is part of the heading, not decoration): `## Backlog #status-backlog`, `## In Progress #status-in-progress`, `## In Testing #status-testing`, `## Done #status-done`, `## Cancelled #status-cancelled`. File mapping: **Backlog** ↔ `backlog.md`, **In Progress** and **In Testing** both ↔ `tasks.md`, **Done** ↔ `changelog.md`, **Cancelled** ↔ `cancelled.md`. **In Testing** is a board-only sub-status: a task moved there is still physically in `tasks.md` (implementation done, pending manual check against `specs/verification.md`) — moving a card into/out of it is a manual board action, not triggered by a `specs/*.md` edit; only Backlog↔In Progress↔Done/Cancelled transitions are driven by (and mirror) an actual file move. Card format: `- [ ] <ID> — <short title> #prio-<priority>` (`#prio-low` / `#prio-medium` / `#prio-high` / `#prio-critical`), checked (`- [x]`) only in **Done** — **Cancelled** cards stay unchecked (closed, but not accomplished); keep the title short — the full description stays in `specs/*.md`, not on the card. Colors are rendered by the user's locally-installed "Colored Tags Wrangler" community plugin off these exact tag strings (its color mapping is configured by the user in Obsidian's plugin settings, not from this repo) — always reuse the existing `#status-*`/`#prio-*` tags verbatim, never invent new ones (a brand-new tag like `#status-cancelled` needs a color assigned by the user in that plugin's settings before it renders like the others). If the `obsidian` MCP server isn't connected (tools missing/erroring), skip the mirror and tell the user instead of failing the whole task update.
...
- The user asks to add, start, finish, cancel, remove, or re-prioritize a task.
